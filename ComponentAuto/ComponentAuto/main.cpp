...
static bool FindBraceRange(
    const std::string& text,
    size_t openBracePos,
    size_t& outBodyBegin,
    size_t& outBodyEnd)
{
    if (openBracePos == std::string::npos || openBracePos >= text.size() || text[openBracePos] != '{')
    {
        return false;
    }
...
static bool FindEnumBodyRange(
    ...
    size_t& outBodyEnd)
{
    const std::string namespaceNeedle = "namespace " + namespaceName;
    const size_t namespaceFound = text.find(namespaceNeedle);
    if (namespaceFound == std::string::npos)
    {
        return false;
    }

    const size_t namespaceOpenBrace = text.find('{', namespaceFound);
    if (namespaceOpenBrace == std::string::npos)
    {
        return false;
    }

    size_t namespaceBodyBegin = 0;
    size_t namespaceBodyEnd = 0;
    if (!FindBraceRange(text, namespaceOpenBrace, namespaceBodyBegin, namespaceBodyEnd))
    {
        return false;
    }

    const std::string enumNeedle = "enum " + enumName;
    const size_t enumFound = text.find(enumNeedle, namespaceBodyBegin);
    if (enumFound == std::string::npos || enumFound >= namespaceBodyEnd)
    {
        return false;
    }

    const size_t enumOpenBrace = text.find('{', enumFound);
    if (enumOpenBrace == std::string::npos || enumOpenBrace >= namespaceBodyEnd)
    {
        return false;
    }

    return FindBraceRange(text, enumOpenBrace, outBodyBegin, outBodyEnd);
}
...
int main(int Argc, char** Argv)
{
    ...
    size_t BodyBegin = 0;
    size_t BodyEnd = 0;
    if (!FindEnumBodyRange(EnumText, "EComponent", "Type", BodyBegin, BodyEnd))
    {
        std::cout << "ERROR: enum class EComponent block not found in enum.h\n";
        std::string Dummy;
        std::getline(std::cin, Dummy);
        return 1;
    }
