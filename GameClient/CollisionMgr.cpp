	...
	for (size_t i = 0; i < vecLeft.size(); ++i)
	{
		...
		for (size_t j = 0; j < vecRight.size(); ++j)
		{
			...
			if (IsCollision(vecLeft[i]->Collider2D(), vecRight[j]->Collider2D(), data))
			{
				if (iter->second)
				{
					vecLeft[i]->Collider2D()->Overlap({ vecRight[j]->Collider2D().Get(), data.HitNormal});
					vecRight[j]->Collider2D()->Overlap({ vecLeft[i]->Collider2D().Get(), data.HitNormal });
				}
				else
				{
					data.HitNormal = -1.f * data.HitNormal;
					vecLeft[i]->Collider2D()->BeginOverlap({ vecRight[j]->Collider2D().Get(), data.HitNormal });
					vecRight[j]->Collider2D()->BeginOverlap({ vecLeft[i]->Collider2D().Get(), data.HitNormal });
				}
			...
			else
			{
				if (iter->second)
				{
					vecLeft[i]->Collider2D()->EndOverlap({ vecRight[j]->Collider2D().Get(), data.HitNormal });
					vecRight[j]->Collider2D()->EndOverlap({ vecLeft[i]->Collider2D().Get(), data.HitNormal });
				}
...
bool CollisionMgr::IsCollision(Ptr<CCollider2D> _LeftCol, Ptr<CCollider2D> _RightCol, CollisionData& data)
{
