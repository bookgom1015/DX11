...
int Engine::Init(HINSTANCE _hInst, UINT _Width, UINT _Height, bool _EditorMode)
{
    ...
    const INT clientPosX = static_cast<INT>((outputWidth - ClientWidth) * 0.5f);
    const INT clientPosY = static_cast<INT>((outputHeight - ClientHeight) * 0.5f);
    ...
    UINT Style = WS_POPUP;
    m_hWnd = CreateWindowW( L"MyGame", L"MyGame", Style, 
